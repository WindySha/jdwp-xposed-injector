...
def is_App_64Bit(jdwp, threadId):
    processClass = jdwp.get_class_by_name("Landroid/os/Process;")
    if processClass is None:
        print("[-] Cannot find class Process")
        return False

    jdwp.get_methods(processClass["refTypeId"])
    is64BitMeth = jdwp.get_method_by_name("is64Bit")
    if is64BitMeth is None:
        print("[-] Cannot find method Process.is64Bit()")
        return False

    buf = jdwp.invokestatic(processClass["refTypeId"], threadId, is64BitMeth["methodId"])

    if buf[0] != chr(TAG_BOOLEAN):
        print("[-] Unexpected returned type: expecting Boolean")
        return True

    result = struct.unpack(">?", buf[1:2])
    print("[+] Process.is64Bit() successful, is64Bit=%s" % result[0])
    return result[0]
...
def runtime_exec(jdwp, args):
    ...
    if args.cmd:
        ...
        packagename = getPackageName2(jdwp, tId)

        is64Bit = is_App_64Bit(jdwp, tId)

        dst_paren_path = "/data/data/" + packagename + "/xposed_injection"
        dst_paren_path_so_path_32 = dst_paren_path + "/arm"
        dst_paren_path_so_path_64 = dst_paren_path + "/arm64"
        dst_plugin_file_path = dst_paren_path + "/plugin"

        native_injector_32_location = dst_paren_path_so_path_32 + "/libnative-injector.so"
        native_injector_64_location = dst_paren_path_so_path_64 + "/libnative-injector.so"

        if args.mode == 0:
            runtime_exec_payload(jdwp, tId, runtimeClass["refTypeId"], getRuntimeMeth["methodId"],
                                 "mkdir -p " + dst_paren_path_so_path_32)
            runtime_exec_payload(jdwp, tId, runtimeClass["refTypeId"], getRuntimeMeth["methodId"],
                                 "mkdir -p " + dst_paren_path_so_path_64)

            tmpLocation = args.loadlib
            command = "cp " + tmpLocation + " " + native_injector_32_location
            runtime_exec_payload(jdwp, tId, runtimeClass["refTypeId"], getRuntimeMeth["methodId"], command)

            tmpLocation = args.loadlib64
            command = "cp " + tmpLocation + " " + native_injector_64_location
            runtime_exec_payload(jdwp, tId, runtimeClass["refTypeId"], getRuntimeMeth["methodId"], command)

            tmpLocation = args.loaderDexPath
            dstLocation = dst_paren_path + "/classes.dex"
            command = "cp " + tmpLocation + " " + dstLocation
            runtime_exec_payload(jdwp, tId, runtimeClass["refTypeId"], getRuntimeMeth["methodId"], command)

            tmpLocation = args.loaderSoPath
            dstLocation = dst_paren_path_so_path_32 + "/libsandhook.so"
            command = "cp " + tmpLocation + " " + dstLocation
            runtime_exec_payload(jdwp, tId, runtimeClass["refTypeId"], getRuntimeMeth["methodId"], command)

            tmpLocation = args.loader64SoPath
            dstLocation = dst_paren_path_so_path_64 + "/libsandhook.so"
            command = "cp " + tmpLocation + " " + dstLocation
            runtime_exec_payload(jdwp, tId, runtimeClass["refTypeId"], getRuntimeMeth["methodId"], command)


        runtime_exec_payload(jdwp, tId, runtimeClass["refTypeId"], getRuntimeMeth["methodId"],
                             "mkdir -p " + dst_plugin_file_path)

        tmpLocation = args.pluginPath
        dstLocation = dst_plugin_file_path + "/xposed_plugin.apk"
        command = "cp " + tmpLocation + " " + dstLocation
        runtime_exec_payload(jdwp, tId, runtimeClass["refTypeId"], getRuntimeMeth["methodId"], command)

        # time.sleep(2)
        if is64Bit:
            loaded_injector_so = native_injector_64_location
        else:
            loaded_injector_so = native_injector_32_location

        runtime_load_payload(jdwp, tId, runtimeClass["refTypeId"], getRuntimeMeth["methodId"], loaded_injector_so)
        # time.sleep(2)
...
def runtime_exec_payload(jdwp, threadId, runtimeClassId, getRuntimeMethId, command):
    ...
    retId = jdwp.unformat(jdwp.objectIDSize, buf[1:1+jdwp.objectIDSize])
    print ("[+] Runtime.exec(\"%s\") successful, retId=%x" % (command, retId))
...
def getPackageName2(jdwp, threadId):
    #
    # This function will invoke ActivityThread.currentPackageName()
    #
    activityThreadClass = jdwp.get_class_by_name("Landroid/app/ActivityThread;")
    if activityThreadClass is None:
        print("[-] Cannot find class android.app.ActivityThread")
    ...
    currentPackageNameMethod = jdwp.get_method_by_name("currentPackageName")
    if currentPackageNameMethod is None:
        print("[-] Cannot find method ActivityThread.currentPackageName()")
        return False

    buf = jdwp.invokestatic(
        activityThreadClass["refTypeId"], threadId, currentPackageNameMethod["methodId"])
    if buf[0] != chr(TAG_STRING):
        print("[-] %s: Unexpected returned type: expecting String" % propStr)
    else:
        retId = jdwp.unformat(jdwp.objectIDSize, buf[1:1 + jdwp.objectIDSize])
        res = cli.solve_string(jdwp.format(jdwp.objectIDSize, retId))
        print("[+]  currentPackageName(): '%s'" % (res))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--loadlib64", dest="loadlib64", type=str, metavar="LIBRARYNAME",
                        help="Specify library to inject into process load")
    parser.add_argument("--loaderDexPath", dest="loaderDexPath", type=str, metavar="DEXFILEPATH",
                        help="Specify load xposed plugin dex to inject into process")
    parser.add_argument("--loaderSoPath", dest="loaderSoPath", type=str, metavar="SOFILEPATH",
                        help="Specify load xposed plugin so to inject into process")
    parser.add_argument("--loader64SoPath", dest="loader64SoPath", type=str, metavar="64SOFILEPATH",
                        help="Specify load xposed plugin so to inject into process")
    parser.add_argument("--pluginPath", dest="pluginPath", type=str, metavar="pluginPath",
                        help="Specify xposed plugin path")
    parser.add_argument("--mode", dest="mode", type=int, metavar="is_no_copy_data_mode",
                        help="Set whether cppy data to data/data dir")
